...
class GenBlock(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_channels=None, ksize=3, pad=1,
        ...
        self.c1 = nn.Conv2d(in_channels, hidden_channels, kernel_size=ksize, padding=pad, bias=False)
        self.c2 = nn.Conv2d(hidden_channels, out_channels, kernel_size=ksize, padding=pad, bias=False)

        self.b1 = ConditionalBatchNorm2d(in_channels, n_classes)
        self.b2 = ConditionalBatchNorm2d(hidden_channels, n_classes)
        
        if self.learnable_sc:
            self.c_sc = nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0, bias=False)
...
class Generator(nn.Module):
    ...
    def _initialize(self):
        ...
        init.xavier_uniform_(self.c7.weight.data)
...
class OptimizedDisBlock(nn.Module):
    def __init__(self, args, in_channels, out_channels, ksize=3, pad=1, activation=nn.ReLU()):
        ...
        self.c1 = nn.Conv2d(in_channels, out_channels, kernel_size=ksize, padding=pad, bias=False)
        self.c2 = nn.Conv2d(out_channels, out_channels, kernel_size=ksize, padding=pad, bias=False)
        self.c_sc = nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0, bias=False)
